module Lti2Tc
  class Lti2Launch
    ...
    def self.launch user, course, resource, link, return_url
      ...
      def tool_proxy_binding_resolver(fieldname)
        if fieldname == 'custom.url'
          tool_guid = resource.tool.key
          ToolSetting.create_uri(tool_guid, 'Context', course_id)
        end
      end
      ...
      link_parameter_json = link_parameters
    ...
    def self.deprecated_lti_parameters
    ...
    def self.known_lti2_parameters
    ...
    def self.url_encode_users(user_list_str)
      user_list = []
