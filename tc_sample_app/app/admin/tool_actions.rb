ActiveAdmin.register_page 'Tool Actions' do
  ...
  content :title => proc{ I18n.t('active_admin.dashboard') } do
    ...
    if method == 'toggle'
      ...
      html_body = Lti2Tc::ToolRegistration::reregister_tool(
        current_admin_user, deployment_request, tool, tool_consumer_profile,
        tool_consumer_profile_wrapper, tc_deployment_url, "#{tc_deployment_url}/admin/tool_actions"
      )
      render :inline => html_body
    end

    tools = Lti2Tc::Tool.order('product_name').to_a
    tools.each do |tool|
      if tool.is_enabled
